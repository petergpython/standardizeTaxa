...
def extract_best_result(list_res):
    ...
    for i in list_res:
        ...
        if match_type != "NoMatch": 
           ...
           source_used = i['names'][0]['bestResult']['dataSourceTitleShort']
           output_name = i['names'][0]['bestResult']["currentName"]
           status = i['names'][0]['bestResult']['taxonomicStatus']
           final.append((input_name, matched_name, match_type, status, output_name, source_used))
        else: 
           final.append((i['names'][0]['name'], 'no_match', 'no_match', 'no_match', 'no_match', 'no_match'))
    return final

def run_batch_queries(df, threshold = 0.8, sources = ['196']):    
    """
...
def standardize(input_df, threshold = 0.8, sources = ['196']):
    """
    ...
    taxa_standardized_df.columns = ['input_name' , 'matched_name', 'match_type', 'status', 'output_name', 'source_used']  
    ...
    results['dataframe'] = input_df
    results['taxa_reconciliation_table'] = taxa_standardized_df
    return results
